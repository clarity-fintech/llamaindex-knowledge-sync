# LlamaIndex Knowledge Sync

> **Moniversive Invariant Static (MIS)** module · settles on **CLRTY-1** (chain **1202**) · compiler **`misc`**  
> Creator **Chandler William Ferguson** · Org [`clarity-fintech/llamaindex-knowledge-sync`](https://github.com/clarity-fintech/llamaindex-knowledge-sync)
...
**LlamaIndex Knowledge Sync** is written entirely in **MIS** (Moniversive Invariant Static, `.mis`) — a
...
```bash
git clone https://github.com/clarity-fintech/llamaindex-knowledge-sync
cd llamaindex-knowledge-sync
...
| `bin/misc mis/kernel/LlamaIndexKnowledgeSyncKernel.mis --check --compact-letters` | Check a single module directly with the compiler. |

Direct compiler invocation (what the Makefile runs under the hood):

```bash
bin/misc mis/kernel/LlamaIndexKnowledgeSyncKernel.mis --check --compact-letters
...
```
mis/kernel/LlamaIndexKnowledgeSyncKernel.mis
mis/packs/augment_context.mis
mis/packs/chunk_index.mis
mis/packs/doc_ingest.mis
mis/packs/rag_retrieve.mis
mis/packs/store_bind.mis
...
mis/commands/LlamaindexKnowledgeSyncCommandCatalog.mis
mis/commands/LlamaindexKnowledgeSyncCommands.mis
mis/commands/LlamaindexKnowledgeSyncNetworkBind.mis
...
`mis/kernel/LlamaIndexKnowledgeSyncKernel.mis` — the kernel declares the invariants the compiler enforces on every check:
...
// Title: LlamaIndex Knowledge Sync Kernel

module LlamaIndexKnowledgeSyncKernel {
  ...
  invariant domain_bound: domain == llamaindex_knowledge_sync;
  ...
  invariant pack_doc_ingest_flag: pack_doc_ingest_bound == true;
  invariant pack_chunk_index_flag: pack_chunk_index_bound == true;
  invariant pack_rag_retrieve_flag: pack_rag_retrieve_bound == true;
  invariant pack_store_bind_flag: pack_store_bind_bound == true;
  invariant pack_augment_context_flag: pack_augment_context_bound == true;
```
...
// Letter-hash + EMBED gates 3..=6 · settlement clrty-1 / chain 1202
// Compile: bin/misc <file>.mis --check --compact-letters
// Title: LlamaIndex Knowledge Sync · AugmentContext

module LlamaIndexKnowledgeSyncAugmentContext {
  ...
  invariant settlement_network_clrty1: settlement_network == clrty_1;
  invariant kernel_is_misc: compiler_kernel == misc;
  invariant pack_augment_context_bound: pack_bound == true;
  invariant catalog_backlink: mis_catalog_bound == true;

  invariant augment_context_081: augment_context_081_bound == true;
  invariant augment_context_082: augment_context_082_bound == true;
  invariant augment_context_083: augment_context_083_bound == true;
  invariant augment_context_084: augment_context_084_bound == true;
  invariant augment_context_085: augment_context_085_bound == true;
  invariant augment_context_086: augment_context_086_bound == true;
```
...
```
CONNECTED clrty-1/1202 llamaindex-knowledge-sync
RPC https://rpc.clarity-fintech.com
MODULES mis/commands/LlamaindexKnowledgeSyncCommands.mis
```
...
{
  "ok": true,
  "module": "LlamaIndexKnowledgeSyncBacklinkIndex",
  "kernel": "misc",
  "invariant_count": 9,
  "outcome_count": 4,
  "typed_letters": 1241,
...
*Reproduce:* `make check` → `"ok": true`, module `LlamaIndexKnowledgeSyncBacklinkIndex`, 9 invariants, 4 outcomes, 1241 typed letters.
...
- [clarity-fintech/llamaindex-knowledge-sync](https://github.com/clarity-fintech/llamaindex-knowledge-sync) — this repository
